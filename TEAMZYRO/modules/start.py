...
"""
    ...
    buttons = [
        [
            InlineKeyboardButton("➕ 𝐀𝐝𝐝 𝐌𝐞 𝐓𝐨 𝐆𝐫𝐨𝐮𝐩", 
...
async def generate_group_start_message(client):
    ...
    caption = (f"🍃 ɪ'ᴍ {bot_user.first_name} 🫧\n"
               f"ɪ sᴘᴀᴡɴ Cʀɪᴄᴋᴇᴛᴇʀs ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ ғᴏʀ ᴜsᴇʀs ᴛᴏ ɢʀᴀʙ.\n"
...
async def start_private_command(client, message):
    ...
    caption, reply_markup = await generate_start_message(client, message)
    ...
    if media.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
        await message.reply_photo(photo=media, caption=caption, reply_markup=reply_markup)
    else:
        await message.reply_video(video=media, caption=caption, reply_markup=reply_markup)

# 🔹 Group Start Handler
@app.on_message(filters.command("start") & filters.group)
async def start_group_command(client, message):
    caption, reply_markup = await generate_group_start_message(client)
    media = random.choice(START_MEDIA)
...
async def back_to_home(client, query: CallbackQuery):
    caption, reply_markup = await generate_start_message(client, query.message)
